...
struct TerminalTable
  ...
  def list_templates(list : Array(Template))
    ...
    puts "#{header_distribution} #{header_version} #{header_architecture}"

    # Then, print the table
    list.each do |template|
      puts "#{template.distribution.ljust(length_distribution)} #{template.version.ljust(length_version)} #{template.architecture.ljust(length_architecture)}"
    end
  end
end
